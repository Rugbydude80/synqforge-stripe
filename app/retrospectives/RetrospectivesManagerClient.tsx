...
export default function RetrospectivesManager({ projectId, organisationId }: { projectId: string; organisationId: string }) {
  ...
  const refreshData = async () => {
    ...
    if (retrosError) {
      ...
      const normalized = (retrosData || []).map((r) => {
        let tags: Record<string, string[]> | null = null;
        if (r.tags && typeof r.tags === 'object') {
          tags = Object.fromEntries(
            Object.entries(r.tags as Record<string, unknown>).map(([k, v]) => [k, Array.isArray(v) ? (v as unknown[]).map(String) : []])
          );
        }
        return { ...r, tags } as Retrospective;
      });
      setRetros(normalized);
    }
  };
  useEffect(() => {
    void refreshData();
    ...
    channel
      .on('postgres_changes', { event: '*', schema: 'public', table: 'sprints', filter: `project_id=eq.${projectId}` }, () => void refreshData())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'retrospectives' }, () => void refreshData())
      .subscribe();
    return () => {
      channel.unsubscribe();
    };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          <select value={selectedSprint} onChange={(e) => setSelectedSprint(e.target.value)} className="border rounded p-2">
          ...
          <button onClick={handleGenerate} disabled={generating || !selectedSprint} className="bg-blue-600 text-white px-4 py-2 rounded disabled:opacity-50">
          ...
          retros.map((retro) => {
            ...
            return (
              <div key={retro.id} className="border rounded p-4 bg-white dark:bg-zinc-800">
                <h2 className="font-semibold text-lg">Retrospective: {sprint ? sprint.name : 'Unknown Sprint'}</h2>
                {editing === retro.id ? (
                  <div className="space-y-2 mt-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      {['what_went_well', "what_didnt", 'ideas'].map((k) => (
                        <input
                          ...
                          placeholder={`${k.replace('_', ' ')} (comma separated)`}
                    ...
                    <div className="flex items-center justify-end gap-2">
                      <button className="text-sm px-2 py-1 rounded border" onClick={() => { setEditing(null); setEditForm({}); }}>
                        Cancel
                      </button>
                      <button className="text-sm px-2 py-1 rounded bg-blue-600 text-white" onClick={saveRetro}>
                        Save
                      </button>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex justify-end mt-2">
                      <button className="text-sm text-blue-600" onClick={() => { setEditing(retro.id); setEditForm({ summary: retro.summary ?? '', notes: retro.notes ?? '', tags: retro.tags ?? {} }); }}>
                        Edit
                      </button>
                    </div>
                  </div>
                )}
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Generated {retro.created_at ? new Date(retro.created_at).toLocaleString() : ''}</p>
              </div>
            );
          })
        ) : (
