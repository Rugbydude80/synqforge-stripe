...
export default function SprintsClient({ projectId }: { projectId: string }) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="rounded border p-4 bg-white dark:bg-zinc-900">
          ...
          <div className="space-y-3">
            ...
            <button className="rounded bg-blue-600 text-white px-4 py-2 disabled:opacity-50" disabled={saving} onClick={submit}>
              {saving ? 'Saving…' : 'Save'}
            </button>
          </div>
        </div>
              ...
              <li key={s.id} className="flex items-center justify-between rounded border p-2">
                <div>
                  <div className="font-medium">
                    <a className="text-blue-600 hover:underline" href={`/sprints/${s.id}`} aria-label={`Open sprint ${s.name}`}>
                      {s.name}
                    </a>
                ...
                <div className="flex items-center gap-2">
                  <button className="text-sm text-blue-600" onClick={() => edit(s)}>
                    Edit
                  </button>
                  {s.status !== 'active' && (
                    <button
